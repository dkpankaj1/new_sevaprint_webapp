                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="email" class="form-control" name="email"
                                value="{{ old('email', $user->email) }}" placeholder="examole@email.com">
                            @error('email')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select name="state" class="form-control">
                                ...
                                @foreach ($country->states as $state)
                                    <option value="{{ $state->name }}"
                                        @if ($state->name === old('state',$user->state)) selected @endif>
                                        {{ $state->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="is_active" class="form-control">
                                <option value="">---select---</option>
                                <option value="1" @if (old('is_active', $user->is_active) === 1) selected @endif>active
                                </option>
                                <option value="0" @if (old('is_active', $user->is_active) === 0) selected @endif>in-active
                                </option>
                            </select>
                            @error('is_active')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
